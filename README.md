...
*   **Google One-Tap**: Instant cross-platform sign-in using Google Identity Services (optimized for Web, Mobile, and Desktop).
*   **Smart Discovery**: Invited members automatically see their groups upon first sign-in via phone number matching.
...
*   **Detailed Audit Logs**: Activity logs track exactly what changed (amount, date, category, or note) with "before and after" details.

### 💳 Deep UPI Integration
*   **Multi-App Support**: launches GPay, PhonePe, Paytm, or BHIM directly from the app.
*   **Dynamic QR Codes**: Instant QR generation for every pending settlement.
*   **Payment Confirmation**: Two-way handshake for payment tracking; balances only clear when the receiver confirms receipt.

### 🛡️ Production & Stability
*   **Responsive Redesign**: Scrollable, safe-area aware modals for payment confirmation and expense entry.
*   **Balance Locks**: Structural safety prevents removing members unless their adjusted balance is exactly ₹0.00.
*   **Multi-Platform Configuration**: Fully configured for Web (managed GSI client IDs) and iOS (URL schemes and permission strings).
...
- **Auth**: Firebase Authentication (Google GSI + Phone Discovery)
- **State Management**: Provider (Centralized `AppState`)
- **Platform Specifics**: `google_sign_in_web`, `url_launcher` (UPI), `flutter_contacts`.
...
## 📂 Project Structure
- `lib/providers/app_state.dart`: The core logic hub handling all financial transactions and sync.
- `lib/utils/settlement_helper.dart`: The mathematical engine for debt simplification.
- `lib/screens/group_settlement_screen.dart`: The immersive group dashboard.
